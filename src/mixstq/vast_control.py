...
def search(gpu: str, max_price: float, min_vram: int, disk: int, limit: int) -> list[dict]:
    query = {
        "rentable": {"eq": True},
        "num_gpus": {"eq": 1},
    ...
    for offer in offers:
        if not offer.get("rentable", False):
            continue
        if (offer.get("num_gpus") or 0) != 1:
            continue
        if (offer.get("gpu_ram") or 0) < min_vram * 1000:
            continue
        if (offer.get("dph_total") or 1e9) > max_price:
            continue
        if (offer.get("disk_space") or 0) < disk:
            continue
        if gpu and offer.get("gpu_name") != gpu.replace("_", " "):
            continue
        selected.append(offer)
    selected.sort(key=lambda o: o.get("dph_total") or 1e9)
    ...
    return [
        {
            ...
            "down_mbps": round(offer.get("inet_down") or 0.0, 1),
            "reliability": round(offer.get("reliability2") or 0.0, 4),
...
def main() -> int:
    ...
    if args.command == "search":
        offers = search(args.gpu, args.max_price, args.min_vram, args.disk, args.limit)
    ...
    if args.command == "create":
        if not args.confirm:
            ...
            return 0
        offers = {o["id"]: o for o in search("", args.max_hourly, args.min_vram, args.disk, 200)}
